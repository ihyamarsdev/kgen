...
func runPreview(args []string) {
	...
	for i := 0; i < len(relPaths); i++ {
		for j := i + 1; j < len(relPaths); j++ {
			if relPaths[j] < relPaths[i] {
	...
	for _, rel := range relPaths {
		...
		rendered, err := renderTemplate(rel, content, chartDir)
		if err != nil {
...
func renderTemplate(rel, content, chartDir string) (string, error) {
	...
	// We don't have the original Config struct, so pass an empty map.
	// This will render the template with zero values for any {{ .Field }}.
